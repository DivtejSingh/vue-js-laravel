          ...
          <v-card outlined>
            <v-simple-table>
              ...
              <draggable tag="tbody" :list="dealbanners" handle=".drag-handle" @end="onDragEnd">
                <tr v-for="item in filteredBanners" :key="item.home_explore_deal_banner_id">
                  <td class="p-2">
                    <img :src="cdn + item.home_explore_deal_banner_image || 'https://via.placeholder.com/50'" width="100" height="75" />
                  </td>
                  <td class="align-middle">
                    {{ item.home_explore_deal_banner_name }}
                  </td>
                  <td>
          ...
          <v-form v-model="fsvalid" @submit.prevent="saveBanner">
            <v-card-text>
              ...
              <v-select dense outlined class="mt-3"
                        ...
                        clearable/>
              <v-text-field
                v-model="defaultItem.home_explore_deal_banner_name"
...
export default {
  ...
  data () {
    return {
      ...
      fillLock       : false,       
      defaultItem    : {
        home_explore_deal_banner_id   : null,
        home_explore_deal_banner_name : '',
        home_explore_deal_banner_image: '',
        main_mcat_id      : null,
        mcat_id           : null,
        msubcat_id        : null,
        mproduct_id       : null
  ...
  computed : {
    ...
    filteredBanners () {
      ...
      return this.dealbanners.filter(b =>
        (b.home_explore_deal_banner_name || '').toLowerCase().includes(term)
      )
    }
  },
  ...
  methods : {
    ...
    async loadBanners () {
      const { data } = await axios.get('/admin/explore-deal-banners/vlist')
      this.dealbanners = data.home_explore_deal_banner
    },
    ...
    async editItem (item) {
      ...
      this.defaultItem = {
        home_explore_deal_banner_id   : item.home_explore_deal_banner_id,
        home_explore_deal_banner_name : item.home_explore_deal_banner_name,
        home_explore_deal_banner_image: '',
      ...
      this.imagePreview = this.cdn + item.home_explore_deal_banner_image
      this.imageName    = item.home_explore_deal_banner_image.split('/').pop()
      this.editedIndex  = item.home_explore_deal_banner_id
      this.fsvalid      = true
      this.addSdialog   = true
    },
    ...
    handleImageUpload (e) {
      ...
      if (!file) return
      this.defaultItem.home_explore_deal_banner_image = file
    ...
    async saveBanner () {
      ...
      fd.append('home_explore_deal_banner_name', this.defaultItem.home_explore_deal_banner_name)
      if (this.defaultItem.home_explore_deal_banner_image instanceof File)
        fd.append('home_explore_deal_banner_image', this.defaultItem.home_explore_deal_banner_image)
      if (this.editedIndex !== -1)
        fd.append('home_explore_deal_banner_id', this.editedIndex)

      const isNew = this.editedIndex === -1
      const url   = isNew ? '/admin/explore-deal-banners/add'
                          : '/admin/explore-deal-banners/update'
    ...
    /* --------------- DRAG REORDER --------------- */
    async onDragEnd () {
      const payload = this.dealbanners.map((it,i)=>({id:it.home_explore_deal_banner_id,position:i+1}))
      try {
        await axios.post('/admin/explore-deal-banners/reorder', payload)
    ...
    async performDelete () {
      ...
      try {
        await axios.post('/admin/explore-deal-banners-delete',
                         {home_explore_deal_banner_id:this.browseBannerToDelete.home_explore_deal_banner_id})
