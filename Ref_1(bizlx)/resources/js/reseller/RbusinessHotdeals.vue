<template>
          ...
          <div class="row pb-3">
            ...
            <div class="col-12 col-sm-4 test-decoration-none">
              <div v-if="hotdeals_count === userDta.hot_deals">
                <v-btn @click="MessageModel1 = true" class="error"  tile large>
                  <v-icon left>mdi-plus</v-icon>CREATE HOT DEAL
                </v-btn>
              </div>
              <div v-else>
                <v-btn @click="openhotdeal" class="error"  tile large>
                  <v-icon left>mdi-plus</v-icon>CREATE HOT DEAL
                </v-btn>
              </div>
            </div>
      ...
      <v-dialog v-model="showModel1" persistent max-width="900">
        <v-card>
          <v-form @submit.prevent="dealAddFormSubmit" ref="form" v-model="valid" lazy-validation>
              ...
              <div class="ms-3 pb-3">
                <v-btn v-if="isLoading" class="my-2 me-3 cred">Loading...</v-btn>
                <v-btn v-else class="my-2 me-3"  type="submit"  color="success" :disabled="!valid">SAVE</v-btn>
  ...
  import RsalesView from './RsalesView.vue';
import RjobsView from './RjobsView.vue';
import RvideosView from './RvideosView.vue';
  export default {
    name: 'BusinessHotdeals',
    components: {
      RsalesView,
    RjobsView,
    RvideosView,
    ...
    data: vm => ({
      ...
      dealTitleRules: [
          ...
          v => (v && v.length >= 8)  || 'Deal title must be greater than 8 characters.',
      ],
      priceFromRules: [
    ...
    methods: {
          ...
          UpdateHandleFileChange() {
            ...
            if(this.editedItem.updateSelectedFiles){
          ...
          getBusinessServices() {
              ...
              axios.get("/api/businesses/hotdeals/get")
                .then((resp) =>{
      ...
      deleteItemConfirm() {
          ...
          if(this.userdata.id != null){ // delete by Reseller after login
            ...
            axios.post("/api/businesses/hotdeals/add", data, config)
              .then((resp) => { // delete
                this.closeDelete();
                  this.text = resp.data.message;
                  this.MessageModel = true,
      ...
      dealAddFormSubmit() {
        if (this.editedIndex > -1) { // edit deals
          ...
          if(this.formData.dealTitle!='' && this.formData.priceFrom!='' && this.formData.dateFrom!='' && this.formData.description!=''){
            this.isLoading = true;
