        ...
        <div class="row pb-3">
          ...
          <div class="col-12 col-sm-4 test-decoration-none">
            <div v-if="videoscount === userData.video">
              <v-btn @click="MessageModel5 = true" color="error" tile large>
                <v-icon left>mdi-plus</v-icon>ADD VIDEO
              </v-btn>
            </div>
            <div v-else>
              <v-btn @click="openvideo" color="error" tile large>
                <v-icon left>mdi-plus</v-icon>ADD VIDEO
              </v-btn>
            </div>
          </div>
...
export default{
  ...
  data: () => ({
    ...
    video_titleRules: [
        ...
        v => v.length >= 8 || 'Video title must be greater than 8 characters.',
    ],
  ...
  methods:{
      ...
      getvideo() {
        axios.post("/api/businesses/videos/getbyadmin",{user_id:this.user_id()})
          .then((resp) =>{
                this.videosData = resp.data.allVideos;
             console.log('fghjkl22',this.videosData);
                this.videoscount = resp.data.allVideos.length;
                this.userData = resp.data.user;
        });
      },
    ...
    videoFormSubmit(){
      if (this.editedIndex > -1) { // edit sale
        ...
        if(this.userdata.id != null){
          ...
          axios.post("/api/update/videobyadmin", data, config)
            .then((resp) => {
              this.close();
                this.isLoading = false;
                this.MessageModel4 = true;
        ...
        if(this.formData.video_title != '' && this.formData.video_description != '' && this.formData.video_url != ''){
          ...
          if(this.userdata.id != null){
            ...
            axios.post("/api/add/videobyadmin", data, config)
              .then((resp) => {
                this.showModel4 = false;
                  this.MessageModel4 = true,
                  this.isLoading = false;
