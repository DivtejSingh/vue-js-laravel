          ...
          <v-form ref="form1" v-model="valid1">
            <v-row>
              ...
              <v-col cols="12" md="12" class="py-0">
                <v-text-field clearable label="Name" v-model="reseller.name" :rules="nameRules" required ></v-text-field>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-text-field clearable label="E-mail address" v-model="reseller.email" :rules="emailRules" required ></v-text-field>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-text-field clearable type="number" label="Enter Mobile" v-model="reseller.mobile_phone" :rules="mobileRules" required ></v-text-field>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-text-field clearable label="Reseller ID" v-model="reseller.username" required readonly></v-text-field>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-text-field clearable label="GST" class="d-none"  required ></v-text-field>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-select clearable :items="items1" label="Select Gender" v-model="reseller.reseller_gender" dense></v-select>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-autocomplete clearable label="Select Your Profession" v-model="reseller.profession_id" :rules="profiaction"
                                :items="allProfessions" item-text="profession" item-value="id" prepend-inner-icon="mdi-tray-arrow-down"
                ></v-autocomplete>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <div>
                  <v-autocomplete clearable :items="allSkills" :rules="skill" v-model="skillId" prepend-inner-icon="mdi-code-tags"
              ...
              <v-col cols="12" md="12" class="py-0">
                <v-autocomplete clearable label="Select State" :rules="stateRules" v-model="stateId" @change="getCities" @selected="getCities()"
                                :items="allstates" item-text="state" item-value="id" prepend-inner-icon="mdi-city"
                ></v-autocomplete>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-autocomplete clearable label="Select City" :rules="cityRules" v-model="reseller.city_id"
                                :items="cities" item-text="city" item-value="id" prepend-inner-icon="mdi-home-city"
                ></v-autocomplete>
              </v-col>
              <v-col cols="12" md="12" class="py-0">
                <v-textarea rows="3" clearable row-height="20" :rules="addressRules" v-model="reseller.address" label="Select Address"></v-textarea>
              </v-col>
            </v-row>
            <div class="text-end">
      ...
      <v-tab-item >
        <v-card color="basil" flat >
          <v-form ref="form2" v-model="valid2">
            ...
            <div class="text-end">
              <v-btn class="my-3 ms-4" color="error" @click="updateresellerPassword" :disabled="loading">UPDATE</v-btn>
            </div>
          </v-form>
        </v-card>
      </v-tab-item>
...
export default {
  ...
  methods:{
    ...
    getResellerprofile() {
      axios.get('/api/reseller/profile')
          .then((resp) =>{
    ...
    updateresellerProfile() {
      ...
      axios.post('/api/update/reseller/profile',data, config)
          .then((resp) => {
            this.loading = false;
            this.snackbar = true;
    ...
    updateresellerPassword() {
      ...
      axios.post('/api/update/reseller/password',data, config)
          .then((resp) => {
            if(data.status === 200){
              this.snackbar = true;
